...
if os.path.exists(settings_path):
    ...
    app_client_id = config.get('DEFAULT', 'client_id')
    app_client_secret = config.get('DEFAULT', 'client_secret')
    drive_tenant_id = config.get('DEFAULT', 'tenant_id')
else:
...
oauth = OAuth2Session(app_client_id, scope=scopes, redirect_uri='http://localhost')
...
token = oauth.fetch_token(token_url, client_secret=app_client_secret, authorization_response=redirect_response)
